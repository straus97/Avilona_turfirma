...
| Текущий функциональный checkpoint | `d66035541e8ab96a801b011168663758181c1e25` |
| Commit | `feat: separate public personal data consent` |
| Родительский commit | `6c5c33bede6530d3aae66a93a27c18e578c1330e` (`fix: align public company details`) |
| Предыдущий documentation/source checkpoint | `e067124c4323e82ab5d10d8e6aa1491029fb767b` (`docs: close stage 12`) |
| PHPUnit full baseline | 704 tests / 3128 assertions, exit code 0 |
| PHPUnit focused baseline (`PublicPersonalDataConsentTest`) | 22 tests / 46 assertions |
| PHP | 8.3.32 (`C:\wamp\bin\php\php8.3.32\php.exe`) |
| PHPUnit DB | SQLite `:memory:` — единственно допустимая для PHPUnit |
| Canonical schema | `turfirma_rebuild_v4`, порт 3308 |
| Canonical migrations | 53 Ran / 0 Pending (последняя независимо проверенная canonical-верификация; Stage 13 не добавлял миграций и не требовал нового canonical-прогона) |
...
| Stage 13 | 🚧 **IN PROGRESS** — production readiness / локальный runtime / публичная поверхность / подготовка к деплою. **НЕ завершён.** |

Эта документация описывает состояние на момент текущего функционального
checkpoint `d6603554` (`feat: separate public personal data consent`),
родитель — `6c5c33be` (`fix: align public company details`). Текущий
...
Stage 10 (уведомления), Stage 11 (security, reliability, performance) и
Stage 12 (dependency upgrades) **завершены** — см. соответствующие разделы
ниже и [`roadmap.md`](roadmap.md). **Stage 13 находится в работе** — см.
раздел «Stage 13 — 🚧 IN PROGRESS» ниже: часть работы завершена и
запушена, значительная часть остаётся.

Исторические значения, приведённые ниже в разделах закрытых этапов
(например, Stage 11/Stage 12 baseline 644 tests / 2861 assertions,
dependency checkpoint `61d2fef9`, функциональный checkpoint `f3279a42`),
сохранены **как исторические** и не описывают текущее состояние
репозитория.
...
Внешний Project Sources / handoff набор в рамках этой documentation-only
задачи **не генерировался и не обновлялся**. Он должен быть сгенерирован и
независимо проверен отдельным guarded шагом из нового documentation HEAD,
созданного этими правками. Фактическое состояние внешнего набора этой
задачей не проверялось; последняя зафиксированная в документации привязка
(на момент закрытия Stage 12) указывала на `85a7b22b` — это историческая
запись, а не подтверждение текущего состояния.
...
В Stage 13 файлы зависимостей (`composer.json`, `composer.lock`,
`package.json`, `package-lock.json`) **не менялись** — таблица остаётся
актуальной.
...
Финальный dependency checkpoint Stage 12 — `61d2fef9`
...
функциональный checkpoint см. в таблице «Текущий checkpoint» в начале
документа.
...
Test и migration baseline **на момент закрытия Stage 12 (историческое
...
требовала нового canonical-прогона). Текущий проверенный baseline —
704 tests / 3128 assertions, см. «Текущий checkpoint» в начале документа.
...
Stage 12 **закрыт**. Документационное закрытие Stage 12 зафиксировано
commit `e067124c` (`docs: close stage 12`) — это предыдущий
documentation/source checkpoint.

## Stage 13 — 🚧 IN PROGRESS. Production readiness и публичная поверхность

Stage 13 **начат и находится в работе**. Ниже — ledger уже завершённой и
запушенной работы, затем явно отделённый список оставшейся работы.
Stage 13 **не закрыт**.

Все Stage 13 коммиты идут линейной цепочкой от Stage 12 closure commit
`e067124c` (`docs: close stage 12`) до текущего HEAD `d6603554`.
Файлы зависимостей и миграции в Stage 13 не менялись.

### Завершённая работа Stage 13

1. **`0316e7c7` — `build: stop tracking generated Vite output`.**
   Сгенерированный вывод сборки под `public/build/` перестал отслеживаться
   Git: из индекса убраны 2288 путей под `public/build/` (в том числе
   устаревшая копия AdminLTE `dist` и файлы, попадавшие туда из-за
   пересечения `publicDir`/`outDir`). `.gitignore` уже содержал
   `/public/build`. В `vite.config.js` `publicDir` изменён с `'public'` на
   `false` — источник ранее задокументированного non-blocking Vite
   предупреждения о пересечении `publicDir`/`outDir` устранён в
   конфигурации. Единственный изменённый (не удалённый из индекса) путь —
   `vite.config.js`. Зависимости не обновлялись, npm/Composer не
   запускались.

2. **`712ff965` — `fix: resolve public browser runtime errors`.**
   Два точечных исправления публичного runtime:
   `resources/views/layouts/main.blade.php` — загрузка `lazysizes`
   переведена с `async` на `defer`; `resources/views/tours/index.blade.php`
   — присвоение `summary.textContent` защищено проверкой на существование
   элемента.

3. **`1b66c548` — `fix: improve mobile responsive layout`.**
   Формы «Контакты» и «Отзывы» переведены с фиксированного `col-6` на
   `col-12 col-md-6`; блок отзыва — с `col-md-1` / `col-md-11` на
   `col-3 col-md-1` / `col-9 col-md-11`; в `public/css/unified.css` добавлен
   переносимый и центрируемый блок `.pagination`.

4. **`3000c298` — `fix: refine tablet responsive layout`.**
   Колонка счётчиков в подвале переведена на `col-6 col-md-12 col-xl-6`
   с `g-0`; блок отзыва уточнён до `col-3 col-md-2 col-lg-1` /
   `col-9 col-md-10 col-lg-11`.

5. **`88d0fcc6` — `fix: clean up public development notices`.**
   С публичного сайта убраны development-facing материалы: модальное окно
   «Важная информация» (текст о том, что сайт находится в стадии активной
   разработки, вместе с личным email для сообщений об ошибках) и все три
   кнопки его вызова; из подвалов `layouts/main.blade.php` и
   `layouts/profile.blade.php` убран персональный кредит разработчика со
   ссылкой на личный профиль. Год в копирайте стал динамическим
   (`{{ date('Y') }}`) вместо жёстко зашитых 2024 / 2023.

6. **`a9bf145d` — `feat: add cookie consent and analytics gating`.**
   Стабильное поведение, на которое можно опираться:
   - имя cookie — `avilona_cookie_consent`;
   - ровно два валидных значения — `v1_all` (аналитика разрешена) и
     `v1_necessary` (только необходимые);
   - `App\Support\CookieConsent` — единственный источник истины
     нормализации: любое отсутствующее, пустое, неизвестное или
     несовпадающее по версии значение (`v0_all`, `v2_all` и т. п.)
     нормализуется в `undecided`;
   - счётчики Яндекс.Метрики, LiveInternet и Top.Mail.Ru рендерятся
     сервером **только** при согласии на аналитику; без согласия их разметка
     в ответ не попадает;
   - публичная информационная страница об использовании cookie:
     `GET /cookies`, имя маршрута `cookies.info`;
   - постоянный элемент «Настройки cookie» в подвале позволяет повторно
     открыть баннер и изменить выбор в любой момент;
   - `avilona_cookie_consent` внесён в `EncryptCookies::$except` —
     единственное исключение, шифрование остальных cookie сохранено;
   - `App\Http\Middleware\CacheResponse` изолирует кэш по согласию: в ключ
     кэша добавляется нормализованное состояние согласия, поэтому
     количество вариантов кэша на URL ограничено тремя и ответ,
     отрисованный для одного состояния согласия, не может быть отдан для
     другого. Сырое значение cookie в ключ не попадает;
   - собственные ad-hoc кэши страниц «Контакты» и «Словарь путешественника»
     (`Cache::has`/`Cache::put` внутри контроллеров) удалены, чтобы они не
     обходили изоляцию по согласию;
   - клиентский скрипт `public/js/cookie-consent.js` отвечает только за UI
     баннера: чтение/запись cookie (`Path=/`, `Max-Age` 1 год,
     `SameSite=Lax`, `Secure` при HTTPS), перезагрузку страницы после
     выбора и повторное открытие баннера. Он не собирает аналитику, не
     отправляет сетевых запросов и не хранит персональных данных;
   - покрытие: `PublicCookieConsentTest` (10 тестов) и
     `CacheResponseConsentIsolationTest` (17 тестов).

7. **`6c5c33be` — `fix: align public company details`.**
   Публичные реквизиты приведены к единому фактическому состоянию:
   - фактический адрес / офис:
     198261, Россия, Санкт-Петербург, ул. Генерала Симоняка, д. 10;
   - юридический (регистрационный) адрес:
     191119, Россия, Санкт-Петербург, ул. Звенигородская, д. 22,
     литера А, офис 053, пом. 7Н;
   - ИНН 7805502454;
   - КПП 784001001 (ранее на странице «Контакты» было ошибочное 780501001);
   - ОГРН 1097847289803.

   Юридический и фактический адреса **намеренно различаются** — это не
   рассинхронизация. На странице «Контакты» они выведены как две отдельные
   подписанные строки; на главной, в шапке и подвале указан именно адрес
   офиса; в трёх шаблонах писем подвал приведён к адресу офиса (ранее там
   встречались смешанные и некорректные комбинации индекса и улицы).
   Покрытие: `PublicCompanyDetailsConsistencyTest` (11 тестов).

8. **`d6603554` — `feat: separate public personal data consent`
   (текущий HEAD).**
   Формы «Главная» и «Контакты» получили два независимых обязательных
   подтверждения вместо одного объединённого:
   - `agree` — принятие Пользовательского соглашения (ссылка на существующий
     PDF Пользовательского соглашения);
   - `personal_data_consent` — отдельное согласие на обработку персональных
     данных;
   - оба серверных правила в `SendHomeRequest` и `SendContactRequest`
     используют `accepted` (ранее `agree` было `required`, что принимало
     любое непустое значение), с отдельными сообщениями об ошибке для
     каждого подтверждения;
   - добавлен публичный маршрут `GET /personal-data-consent`, имя
     `personal_data_consent.info`, отдельная visitor-facing страница
     `resources/views/legal/personal-data-consent.blade.php`;
   - страница согласия доступна анонимно и ссылается на существующую
     Политику в отношении защиты и обработки персональных данных (PDF);
     новых юридических PDF/DOCX не добавлялось;
   - **evidence согласия не сохраняется**: ни отметки времени, ни версии,
     ни объёма, ни IP — только валидация факта согласия в запросе;
   - миграций не добавлялось, схема БД не менялась;
   - «Отзывы», «Регистрация» и «Бронирование» в этом слайсе **намеренно не
     менялись**;
   - покрытие: `PublicPersonalDataConsentTest` (22 теста / 46 assertions).

### Проверенный тестовый baseline Stage 13

```text
PHP:              8.3.32
PHPUnit DB:       SQLite :memory:
Full suite:       704 tests / 3128 assertions
Exit code:        0
```

Арифметика этого baseline независимо сверена:

- предыдущий проверенный baseline непосредственно перед слайсом
  personal-data-consent: 682 tests / 3081 assertions;
- новый `PublicPersonalDataConsentTest`: 22 tests / 46 assertions;
- новый публичный маршрут увеличивает количество assertions существующего
  `PublicReviewDetailRouteConsistencyTest` ровно на один, потому что этот
  тест выполняет по одному assertion на каждый зарегистрированный маршрут
  (`foreach (Route::getRoutes() ...)`);
- итог: 682 + 22 = 704 tests, 3081 + 46 + 1 = 3128 assertions.

Это полностью объяснённая величина, а не необъяснённое расхождение.

### Локальный runtime и браузерные evidence Stage 13

Для Stage 13 настроен изолированный локальный WAMP-стенд браузерной
проверки Avilona:

```text
avilona.local -> 127.0.0.2
```

Обычные проекты на `localhost` при этом не затрагиваются. Приватные файлы
браузерных evidence в репозиторий не копируются.

Результат последней браузерной QA для раздельного согласия на обработку
персональных данных на «Главной» и «Контактах»:

- desktop1440 (1440×1000) — PASS;
- mobile390 (390×844) — PASS;
- проверенные маршруты: `/`, `/contacts`, `/personal-data-consent`;
- два отдельных обязательных элемента согласия подтверждены в реальном DOM;
- независимость состояний «отмечено» у этих двух элементов подтверждена;
- ссылка на Пользовательское соглашение проверена;
- ссылка на страницу согласия на обработку персональных данных проверена;
- страница согласия доступна анонимно;
- ссылка на PDF политики проверена;
- горизонтального переполнения нет;
- исключений локального runtime нет;
- ошибок консоли нет;
- отправка формы во время браузерной QA не выполнялась;
- запросов к провайдерам не выполнялось;
- репозиторий остался неизменным.

### Оставшаяся работа Stage 13

Ни один из пунктов ниже не реализован; ни одно из перечисленных решений не
считается утверждённым. Подробности и границы — в
[`roadmap.md`](roadmap.md) → «Stage 13».

- **A. Согласие на обработку персональных данных / публикацию для отзывов.**
  Форма отзывов по-прежнему содержит старую объединённую формулировку и
  правило `agree => required`. Содержимое отзыва и имя могут публиковаться
  публично; email формой требуется, но текущей реализацией
  (`Review\CreateController`) не сохраняется. Требуется отдельный
  рассмотренный slice. Финальные юридические формулировки в этих
  канонических документах не фиксируются.
- **B. Согласие и применимость политики для публичной регистрации.**
  Публичная регистрация собирает имя, email и пароль; текущая
  документальная/политическая база не покрывает явно цель обработки
...
- **C. Решение о хранении evidence согласия.** Текущее согласие на
  ...
  хранение отметки времени, версии, объёма и факта отзыва **не
  реализовано** и не должно представляться как принятое решение. Любая
  работа со схемой/миграциями остаётся отдельно gated.
...
намеренно не доводится до финального вида в рамках текущей Stage 13 работы
по согласиям и юридическим материалам.
...
Реальные запросы к провайдерам и интеграции в рамках Stage 13
документационной работы не инициировались.
...
- Зафиксированное в Stage 12 предупреждение Vite о пересечении
  `publicDir`/`outDir` относится к конфигурации, действовавшей до
  `0316e7c7`: с этого коммита `publicDir` установлен в `false`, поэтому
  пересечения в конфигурации больше нет. Отдельный подтверждающий
  production-build прогон в рамках Stage 13 документации не выполнялся.
...
Stage 0–12 закрыты. **Stage 13 находится в работе** и не закрыт.

1. Завершённая и запушенная часть Stage 13 (repository/runtime cleanup
   сборки Vite, исправление публичного браузерного runtime, мобильная и
   планшетная адаптивность, очистка development-facing материалов
   публичного сайта, cookie consent и гейтинг аналитики, согласование
   публичных реквизитов компании, раздельное согласие на обработку
   персональных данных на «Главной» и «Контактах») зафиксирована в разделе
   «Stage 13 — 🚧 IN PROGRESS» выше. Текущий функциональный checkpoint —
...
2. Ближайшее отдельное guarded действие — **Project Sources refresh** из
   ...
   roadmap export, new-chat prompt, manifest и source archive. В рамках
   этой задачи они **не генерировались**.
3. Далее — оставшаяся работа Stage 13 в порядке разделов A–H («Оставшаяся
   работа Stage 13» выше и [`roadmap.md`](roadmap.md) → «Stage 13»).
   Пункты A, B и C требуют отдельных контентных/юридических/бизнес-решений
   до реализации и не считаются утверждёнными.
4. Пункт D (гостевой сценарий бронирования) — отдельный функциональный
   дефект, его нельзя смешивать с работой по согласиям.
5. Финальный независимый аудит сайта (пункт H) выполняется **после**
   production-валидации и ещё не проводился.
6. Как и для каждого предыдущего слайса и этапа — миграции, canonical DB
   операции, production-интеграции и деструктивные действия выполнять
   только по отдельному guarded плану. Полный список жёстких ограничений —
