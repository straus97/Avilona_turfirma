                       ...
                       style="text-decoration: none; transition: all 0.2s; position: relative;">
                        <div class="chat-thread__row">
                            ...
                            <div class="chat-thread__body">
                                ...
                                <div class="chat-thread__status d-flex flex-wrap align-items-center gap-2 mt-1" data-chat-thread-badges>
    ...
    <div class="col-md-8">
        @if($currentBooking)
            <div class="card-custom cabinet-chat-pane cabinet-chat-pane--window" data-chat-window tabindex="-1">
                ...
                @if(! $adminIsAssignedHandler)
                    {{-- Режим A: администратор-наблюдатель. Композер и опрос
                         намеренно отсутствуют (см. комментарий выше) — сообщаем
                         об этом явно, а не молча скрываем форму отправки. --}}
                    <div class="border-top pt-3">
                        <p class="text-muted small mb-0" role="status">
                            <i class="bi bi-eye" aria-hidden="true"></i>
                            Режим просмотра: заявку ведёт другой менеджер, отправка сообщений недоступна.
                        </p>
                    </div>
                @endif
