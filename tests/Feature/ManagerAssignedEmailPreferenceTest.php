...
use App\Listeners\SendManagerAssignedNotification;
...
/**
 * Проверяет контракт SendManagerAssignedNotification: письмо о назначении
 * менеджера ставится в очередь независимо для владельца заявки и для
 * назначенного менеджера, и только тому из них, кто (1) присутствует,
 * (2) не имеет технического email, (3) явно не отключил email_notifications
 * и (4) явно не отключил booking_updates. Настройки по умолчанию (null,
 * отсутствующие ключи) трактуются как «включено».
 *
 * Слушатель вызывается напрямую (без диспетчера событий и без HTTP-слоя),
 * чтобы не дублировать уже существующие тесты авторизации/переназначения
 * в BookingReassignmentNotificationTest.
 */
class ManagerAssignedEmailPreferenceTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_default_null_settings_queue_mail_to_owner_and_manager(): void
    {
        ...
        Mail::assertQueued(
            ManagerAssignedMail::class,
            fn (ManagerAssignedMail $mail): bool =>
    ...
    public function test_owner_email_notifications_false_suppresses_only_owner_mail(): void
    {
        ...
        Mail::assertNotQueued(
            ManagerAssignedMail::class,
            fn (ManagerAssignedMail $mail): bool => $mail->recipient->id === $owner->id
        );
    ...
    public function test_manager_email_notifications_false_suppresses_only_manager_mail(): void
    {
        ...
        Mail::assertNotQueued(
            ManagerAssignedMail::class,
            fn (ManagerAssignedMail $mail): bool => $mail->recipient->id === $manager->id
        );
    ...
    // -----------------------------------------------------------------------
    // 6. Несвязанный new_messages=false не подавляет письмо о назначении менеджера
    // -----------------------------------------------------------------------

    public function test_unrelated_new_messages_false_does_not_suppress_manager_assigned_mail(): void
    {
        $owner   = $this->makeUser(Role::TOURIST);
        $manager = $this->makeUser(Role::MANAGER);
        $booking = $this->makeBookingFor($owner, $manager->id);
    ...
    {
        (new SendManagerAssignedNotification())->handle(
            new ManagerAssigned($booking->fresh())
        );
    }
}
